...
function PortfolioEditorPageInner() {
  const searchParams = useSearchParams();
  const initialTab = resolvePortfolioTab(searchParams.get("tab"));
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (settingsRes.ok) {
          const { settings: savedSettings } = (await settingsRes.json()) as {
            settings?: PortfolioSettings;
          };
          if (savedSettings?.slug) {
            setSettings(savedSettings);
          }
        }

        if (projectsRes.ok) {
          const { projects: savedProjects } = (await projectsRes.json()) as {
