...
function CreatePageInner() {
  ...
  useEffect(() => {
    fetch("/api/check-pro")
      .then((res) => res.json())
      .then((result: { isPro: boolean }) => {
        if (result.isPro) setUserIsPro(true);
      })
      .catch(() => {
  ...
  // Reset ATS banner when layout changes
  useEffect(() => {
    setATSBannerDismissed(false);
  }, [data.layoutId]);
  ...
  return (
    <div className="flex h-screen flex-col bg-[#faf8f5]">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-[#e8e2da] bg-white px-4 shadow-sm">
        ...
        <div className="hidden sm:flex items-center gap-3">
            ...
            onClick={() => {
              if (!userIsPro) {
                setProGateFeature({
                  feature: "Upload PDF",
                  description:
                    "Parse your existing resume PDF and auto-fill all fields instantly.",
                });
                return;
              }
          ...
          >
            ...
            Upload PDF
            {!userIsPro && (
              <span className="ml-1 rounded-sm bg-gold/15 px-1.5 py-0.5 text-[0.6rem] font-bold tracking-wider text-gold uppercase">
                Pro
              </span>
            )}
          </button>
            ...
            onClick={() => {
              if (!userIsPro) {
                setProGateFeature({
                  feature: "ATS Check",
                  description:
                    "Scan your resume for ATS compatibility issues and get actionable tips to improve your score.",
                });
                return;
              }
            ...
            onClick={() => {
              if (!userIsPro) {
                setProGateFeature({
                  ...
                  description:
                    "Generate a tailored cover letter from your resume data, ready to customize and send.",
                });
                return;
              }
        ...
        (data.layoutId === "modern" ||
          ...
          <div className="hidden md:flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 mx-4 mt-2 mb-1">
            ...
            <p className="flex-1 text-xs text-amber-800">
              <span className="font-semibold">Heads up:</span> This layout uses
              multiple columns that some ATS systems may struggle to parse. Use
              the <span className="font-semibold">ATS-Safe PDF</span> button
              when applying to jobs.
            </p>
            <button
              onClick={() => setATSBannerDismissed(true)}
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          <div className="mx-auto max-w-2xl p-6">
            <ResumeForm data={data} onChange={handleChange} isPro={userIsPro} />
          </div>
        </div>
      ...
      {proGateFeature && (
        <ProGate
          feature={proGateFeature.feature}
          description={proGateFeature.description}
