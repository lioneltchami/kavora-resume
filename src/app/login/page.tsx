		...
		<main className="flex min-h-screen items-center justify-center bg-bg px-4 py-12">
			...
			<div className="w-full max-w-[420px]">
				<div className="animate-fade-in-up rounded-sm border border-border bg-white px-8 py-10 shadow-[0_2px_24px_rgba(0,0,0,0.04)] sm:px-10 sm:py-12">
					...
					{error === "resend" && (
						<div className="mt-6 rounded-sm border border-red-200 bg-red-50 px-4 py-3 text-center text-sm text-red-700">
							Could not resend the confirmation email. Check the address and try
							again.
						</div>
					)}
					...
					{awaitingConfirm && (
						<form className="mt-6 space-y-3 rounded-sm border border-border-light bg-[#faf8f5] px-4 py-4">
							...
							<button
								...
								type="submit"
								className="w-full rounded-sm border border-navy px-4 py-2.5 text-[0.875rem] font-medium text-navy transition-all hover:bg-navy hover:text-white"
							>
					...
					{view === "forgot" ? (
						<form className="mt-8 space-y-4">
							...
							<button
								...
								type="submit"
								className="w-full rounded-sm bg-navy px-4 py-2.5 text-[0.875rem] font-medium text-white transition-all hover:-translate-y-px hover:bg-navy-light hover:shadow-[0_4px_16px_rgba(30,42,58,0.2)]"
							>
								Send Reset Link
							</button>
							<p className="pt-1 text-center text-[0.8125rem] text-text-muted">
								<Link
							...
							<form className="space-y-4">
								...
								<div>
									<div className="mb-1.5 flex items-center justify-between">
										<label
											htmlFor="password"
											className="block text-[0.75rem] font-medium tracking-wide text-text-muted"
										>
											Password
										</label>
										<Link
											href={`/login?view=forgot&next=${encodeURIComponent(next)}`}
											className="text-[0.75rem] text-gold transition-colors hover:text-navy"
										>
											Forgot password?
										</Link>
									</div>
