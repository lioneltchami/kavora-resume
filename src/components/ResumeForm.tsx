...
async function fetchSuggestion(
	...
	if (!res.ok) {
		const err: SuggestErrorResponse = await res
			.json()
			.catch(() => ({ error: "Request failed" }));
		if (res.status === 403 && err.code === "PRO_REQUIRED") {
			const proError = new Error("PRO_REQUIRED");
			(proError as Error & { code: string }).code = "PRO_REQUIRED";
			throw proError;
		}
...
function SuggestButton({
	...
	const isDisabled =
		loading || (!isPro && remaining !== undefined && remaining <= 0);
...
export default function ResumeForm({
	...
	const [showSuggestUpgrade, setShowSuggestUpgrade] = useState(false);
	...
	const handleSuggestSummary = useCallback(async () => {
		...
		try {
			...
			if (!isPro && result.remaining !== undefined) {
				setSuggestionsRemaining(result.remaining);
			} else if (!isPro) {
				setSuggestionsRemaining((prev) => Math.max(0, prev - 1));
			}
		} catch (err: unknown) {
			const error = err as Error & { code?: string };
			if (error.code === "PRO_REQUIRED") {
				setSuggestionsRemaining(0);
				setShowSuggestUpgrade(true);
			} else {
				alert(
			...
			try {
				...
				if (result.suggestions && Array.isArray(result.suggestions)) {
					onChange({
						...
						experience: data.experience.map((e) =>
							e.id === expId
								? { ...e, bullets: result.suggestions as string[] }
								: e,
						),
					});
				}
				...
				<div className="rounded-lg border border-gold/30 bg-gold/5 px-4 py-3">
					<div className="flex items-start gap-3">
						...
						<div className="flex-1">
							...
							<a
								href="/pricing"
								className="mt-2 inline-block rounded bg-gold px-3 py-1 text-xs font-semibold text-white transition-colors hover:bg-gold-dark"
							>
								Upgrade to Pro
							</a>
						</div>
			...
			>
				<div className="flex items-center justify-between mb-1.5">
					...
					<SuggestButton
						...
						remaining={isPro ? undefined : suggestionsRemaining}
						isPro={isPro}
					/>
				</div>
				<textarea
